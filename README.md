...
---
### 🛠 5. 구현 상세
- GPIO 인터럽트를 활용해 입력 이벤트를 감지하고,
  이벤트 발생 시 메인 제어 루프에서 인증 로직을 처리하는 구조로 설계
- SPI 통신 기반 RFID 데이터 수신 흐름을 이해하고,
  통신 타이밍에 따른 인식 안정성을 고려해 제어 로직 구성
- 메인 루프 기반 제어 구조에서 상태 변수를 활용해
  입력 → 판단 → 출력 흐름이 명확한 임베디드 제어 시스템 구현
